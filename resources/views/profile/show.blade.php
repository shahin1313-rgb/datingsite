...
@section('content')
<div
    class="container mx-auto px-4 py-12 transition-colors duration-300"
>
    <div class="flex justify-center">
        <div class="w-full md:w-2/3 lg:w-1/2">
            <div
                class="bg-white dark:bg-slate-900 shadow-2xl rounded-3xl overflow-hidden border border-pink-100 dark:border-slate-800 transition-all duration-300"
            >
                <div
                    class="h-32 bg-gradient-to-r from-pink-400 to-rose-400 dark:from-pink-600 dark:to-rose-600"
                ></div>
                ...
                <div class="relative px-6 pb-6">
                    <div class="flex justify-center">
                        ...
                        @else
                            <div
                                class="w-40 h-40 rounded-full bg-pink-50 dark:bg-slate-800 border-4 border-white dark:border-slate-900 shadow-lg -mt-20 flex items-center justify-center text-pink-300 dark:text-slate-600 z-10"
                            >
                                <svg
                    ...
                    <div class="text-center mt-4">
                        <h2
                            class="text-3xl font-extrabold text-gray-800 dark:text-slate-100"
                        >
                            {{ $user->name }}
                        </h2>

                        <p
                            class="text-pink-500 dark:text-pink-400 font-medium flex justify-center items-center gap-1 mt-1 mb-4"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                class="h-4 w-4"
                        ...
                        >
                            ...
                            @if (
                                ...
                                <form
                                    id="like-form"
                                    action="{{ route('like.store', ['likedUserId' => $user->id]) }}"
                                    method="POST"
                                    class="flex-shrink-0"
                                >
                                    @csrf
                                    ...
                                    >
                                        <i
                                            id="like-icon"
                                            class="{{ $hasLiked ? 'fa' : 'far' }} fa-heart text-xl"
                                        ></i>
                                    </button>
                                </form>
                            @endif
                    ...
                    >
                        ...
                        >
                            <span
                                class="font-bold text-pink-600 dark:text-pink-400 block mb-1 text-base"
                            >
                                📝 بیوگرافی:
                            </span>

                            <p
                                class="italic leading-relaxed text-gray-700 dark:text-slate-400"
                            >
                                {{ $user->bio ?? 'توضیحاتی ثبت نشده است.' }}
                            </p>
                        </div>
                    </div>
...
<div
    id="reportModal"
    class="fixed inset-0 z-50 flex items-center justify-center hidden p-4 transition-all"
>
    <div
    ...
    <div
        class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl z-50 w-full max-w-md overflow-hidden transform transition-all border border-transparent dark:border-slate-800"
    >
        <div
            class="bg-rose-50 dark:bg-rose-950/30 px-6 py-4 border-b border-rose-100 dark:border-rose-900/50 flex justify-between items-center"
        >
            <h3
                class="text-xl font-bold text-rose-700 dark:text-rose-400"
            >
                گزارش تخلف کاربر
            </h3>
            ...
            <button
                type="button"
                data-report-close
                class="text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-200 text-2xl"
                aria-label="بستن"
            >
                &times;
            </button>
        ...
        <div class="p-6">
            ...
            >
                ...
                <label
                    for="reason_select"
                    class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2"
                >
                    علت اصلی گزارش:
                </label>
                ...
                <select
                    ...
                    id="reason_select"
                    class="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-slate-200 rounded-xl p-3 mb-4 outline-none focus:ring-2 focus:ring-pink-400"
                >
                    <option value="مزاحمت">
                        مزاحمت در چت
                    </option>

                    <option value="محتوای نامناسب">
                        پروفایل یا عکس نامناسب
                    </option>
                ...
                <textarea
                    ...
                    placeholder="لطفاً دلیل خود را بنویسید تا ادمین بررسی کند..."
                    required
                ></textarea>
...
<script nonce="{{ \Illuminate\Support\Facades\Vite::cspNonce() }}">
    document.addEventListener('DOMContentLoaded', () => {
        ...
        likeForm?.addEventListener('submit', ajaxLike);

        document
            .querySelector('[data-report-open]')
        ...
        .then((response) => {
            ...
            if (icon.classList.contains('far')) {
                ...
                button.className =
                    ...
                    'shadow-lg shadow-red-200 dark:shadow-none ' +
                    'hover:bg-red-600 transition duration-300 ' +
                    'transform hover:-translate-y-0.5 flex ' +
                    'items-center justify-center w-12 h-12';

                button.title = 'برداشتن لایک';
            } else {
                icon.className = 'far fa-heart text-xl';

                button.className =
                    'p-3 bg-pink-50 dark:bg-slate-800 ' +
                    'text-pink-600 dark:text-pink-400 border ' +
                    'border-pink-200 dark:border-slate-700 ' +
                    'rounded-2xl hover:bg-pink-600 ' +
                    'hover:text-white dark:hover:bg-pink-600 ' +
